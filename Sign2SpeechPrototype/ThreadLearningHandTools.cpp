...
ThreadLearningHandTools::ThreadLearningHandTools(mutex *mBW, bool* pg, bool* pgr, vector<vector<pair<string, long>>>* bW, string * s, int * nb, condition_variable *cS, condition_variable *cD, recordingDialog* rd) : ThreadLearning(mBW,pg,bW,cD)
{
	wordMeaning = s;
	cond_var_gui = cS;
	nbGestures = nb;
		...
		if ((ct.getSenseManager())->Init() == PXC_STATUS_NO_ERROR)
		{
			...
			while (*program_on) {
				...
				if (*program_on && (*program_on_recording)) {
					...
					while ((ct.getSenseManager())->AcquireFrame(true) == PXC_STATUS_NO_ERROR && (*program_on) && (*program_on_recording))
					{
